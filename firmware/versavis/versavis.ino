...
/* ----- Cameras ----- */
Camera cam0(&nh, CAM0_TOPIC, CAM0_RATE, timer_cam0, CAM0_TYPE, CAM0_TRIGGER_PIN, CAM0_EXPOSURE_PIN, false);
Camera cam1(&nh, CAM1_TOPIC, CAM1_RATE, timer_cam1, CAM1_TYPE, CAM1_TRIGGER_PIN, CAM1_EXPOSURE_PIN, false);
Camera cam2(&nh, CAM2_TOPIC, CAM2_RATE, timer_cam2, CAM2_TYPE, CAM2_TRIGGER_PIN, CAM2_EXPOSURE_PIN, false);
...
/* ----- ROS ----- */
  ...
  while (!cam0.isInitialized() || !cam1.isInitialized() || !cam2.isInitialized())
  {
  ...
  REG_GCLK_CLKCTRL = static_cast<uint16_t>(GCLK_CLKCTRL_CLKEN | GCLK_CLKCTRL_GEN_GCLK0 | GCLK_CLKCTRL_ID_TCC0_TCC1);
  ...
  REG_GCLK_CLKCTRL = static_cast<uint16_t>(GCLK_CLKCTRL_CLKEN | GCLK_CLKCTRL_GEN_GCLK0 | GCLK_CLKCTRL_ID_TC4_TC5);
  while (GCLK->STATUS.bit.SYNCBUSY == 1)
  {
    ;  // wait for sync
  }
  ...
  attachInterrupt(digitalPinToInterrupt(cam0.exposurePin()), exposureEnd0, FALLING);
  attachInterrupt(digitalPinToInterrupt(cam1.exposurePin()), exposureEnd1, FALLING);
  attachInterrupt(digitalPinToInterrupt(cam2.exposurePin()), exposureEnd2, FALLING);
  interrupts();
...
void TCC0_Handler()
{  // Called by cam0_timer for camera 0 trigger.
  cam0.triggerMeasurement();
}

void TCC1_Handler()
{  // Called by cam1_timer for camera 1 trigger.
  cam1.triggerMeasurement();
}

void TC3_Handler()
{  // Called by cam2_timer for camera 2 trigger.
  cam2.triggerMeasurement();
}

void TC5_Handler()
{  // Called by imu_timer for imu trigger.
