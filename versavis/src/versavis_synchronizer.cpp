...
namespace versavis
{
VersaVISSynchronizer::VersaVISSynchronizer(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private)
  : nh_(nh)
  , nh_private_(nh_private)
  , image_transport_(nh)
  , received_first_camera_info_(false)
  , kMaxImageCandidateLength(10)
  , kMinSuccessfullConsecutiveMatches(4)
  , kMaxImageDelayThreshold(0.1)
  , slow_publisher_image_counter_(0)
  , init_number_(0)
  , initialized_(false)
  , publish_slow_images_(false)
  , publish_every_n_image_(10)
  , forward_camera_info_(false)
{
  ...
  image_sub_ = nh_.subscribe(driver_topic_, 10u, &VersaVISSynchronizer::imageCallback, this);
  ...
  {
    camera_info_sub_ = nh_.subscribe(camera_info_sub_topic_, 10u, &VersaVISSynchronizer::cameraInfoCallback, this);
    ROS_INFO("Subscribing to camera info topic %s.", camera_info_sub_topic_.c_str());
  }

  // Subscriber for the image time message containing both the corrected
  // timestamp and the sequence number.
  image_time_sub_ = nh_.subscribe(image_time_sub_topic_, 10u, &VersaVISSynchronizer::imageTimeCallback, this);
  ...
  ROS_INFO("Publishing initialization status to %s.", initialized_pub_topic_.c_str());
  ...
  if (forward_camera_info_)
  {
    camera_info_fast_pub_ = nh_.advertise<sensor_msgs::CameraInfo>(camera_info_fast_pub_topic_, 10u);
    ROS_INFO_STREAM("Publishing camera info to " << camera_info_fast_pub_topic_);

    if (publish_slow_images_)
    {
      camera_info_slow_pub_ = nh_.advertise<sensor_msgs::CameraInfo>(camera_info_slow_pub_topic_, 10u);
      ROS_INFO_STREAM("Publishing (slow) camera info to " << camera_info_slow_pub_topic_);
    }
  }
}
...
void VersaVISSynchronizer::associateTimeStampsAndCleanUp()
{
  ...
  while (image_idx != image_candidates_.end())
  {
    ...
    {
      ROS_WARN("%s: Deleted old image message %ld.", image_fast_pub_topic_.c_str(), image_idx->number);
      image_idx = image_candidates_.erase(image_idx);
    }
    else
    {
  ...
  while (image_time_idx != image_time_stamp_candidates_.end())
  {
    ...
    {
      ROS_WARN("%s: Deleted old image time message %ld.", image_fast_pub_topic_.c_str(),
               image_time_idx->number + offset_);
      image_time_idx = image_time_stamp_candidates_.erase(image_time_idx);
    }
    else
    {
...
void VersaVISSynchronizer::imageCallback(const image_numbered_msgs::ImageNumbered &image_msg)
{
  std::lock_guard<std::mutex> mutex_lock(mutex_);
  ROS_INFO_ONCE("%s: Received first message with %s encoding.", image_fast_pub_topic_.c_str(),
  ...
  if (initialized_)
  {
    ...
    if (image_candidates_.size() > kMaxImageCandidateLength)
    {
      ...
      ROS_WARN("%s: Image candidates buffer overflow at %ld.", image_fast_pub_topic_.c_str(), image_msg.number);
    }
  }
  else
  {
    ...
    ROS_INFO("%s: Current offset %ld with time offset %0.5f [s]", image_fast_pub_topic_.c_str(), offset,
    ...
    if (ros::Time::now().toSec() - init_timestamp_.toSec() < kMaxImageDelayThreshold && offset == offset_)
    {
      ++init_number_;
      if (init_number_ >= kMinSuccessfullConsecutiveMatches)
      {
        ROS_INFO("%s: Initialized with %ld offset.", image_fast_pub_topic_.c_str(), offset);
        initialized_ = true;
        std_msgs::Bool init_msg;
        init_msg.data = true;
        initialized_pub_.publish(init_msg);

        ROS_INFO_STREAM("Versavis node '" << nh_private_.getNamespace() << "' confirmed initialization of sync!");
      }
    }
    else
    {
...
void VersaVISSynchronizer::cameraInfoCallback(const sensor_msgs::CameraInfo &camera_info_msg)
{
  std::lock_guard<std::mutex> mutex_lock(mutex_);
  ROS_INFO_ONCE("%s: Received first camera info message.", camera_info_sub_topic_.c_str());
  received_first_camera_info_ = true;
  camera_info_msg_ = camera_info_msg;
}

void VersaVISSynchronizer::imageTimeCallback(const versavis::TimeNumbered &image_triggered_time_msg)
{
  std::lock_guard<std::mutex> mutex_lock(mutex_);
  ROS_INFO_ONCE("%s: Received first image time stamp message.", image_fast_pub_topic_.c_str());
  if (initialized_)
  {
    ...
    if (image_time_stamp_candidates_.size() > kMaxImageCandidateLength)
    {
      ...
      ROS_WARN("%s: Time candidates buffer overflow at %ld.", image_fast_pub_topic_.c_str(),
               image_triggered_time_msg.number);
    }
  }
  else
  {
...
void VersaVISSynchronizer::publishImg(const image_numbered_msgs::ImageNumbered &image_msg)
{
  if (image_msg.image.header.stamp.toSec() == 0)
  {
    ROS_WARN("%s: Zero timestamp for %ld.", image_fast_pub_topic_.c_str(), image_msg.number);
    return;
  }

  if (image_msg.image.header.stamp.toSec() <= last_stamp_.toSec())
  {
    ROS_WARN("%s: Non-increasing timestamp for %ld.", image_fast_pub_topic_.c_str(), image_msg.number);
    return;
  }
  if (image_msg.number > last_image_number_ + 1)
  {
    ROS_WARN("%s: Skipped %ld frame for unknown reasons.", image_fast_pub_topic_.c_str(),
  ...
  if (forward_camera_info_)
  {
    ...
    {
      ROS_WARN_THROTTLE(2.0, "No camera info received yet, will not publish image.");
    }
  }
  ...
  if (publish_slow_images_)
  {
    ...
    if (slow_publisher_image_counter_ >= publish_every_n_image_)
    {
      if (forward_camera_info_)
      {
        if (received_first_camera_info_)
...
bool VersaVISSynchronizer::readParameters()
{
  ...
  nh_private_.param<bool>("publish_slow_images", publish_slow_images_, publish_slow_images_);
  nh_private_.param<int>("publish_every_n_image", publish_every_n_image_, publish_every_n_image_);
  ...
  ROS_INFO("IMU has a calibrated dalay of %0.0f us.", imu_offset_.toSec() * 1e6);
  return true;
}
